...
import { puedeCorrerElMotor } from "@/lib/candadoMotor";
...
/** Lo poco que este archivo necesita saber de la respuesta de Gemini. */
type RespuestaGemini = {
  candidates?: { content?: { parts?: { text?: string }[] } }[];
  error?: { message?: string };
};

/** Cada trozo que se le manda a Gemini: texto suelto o un archivo. */
type ParteGemini =
  | { text: string }
...
async function llamarGeminiConReintentos(
  ...
  body: unknown,
  maxIntentos = 3
): Promise<{ ok: boolean; data: RespuestaGemini | null }> {
  let ultimoResultado: { ok: boolean; data: RespuestaGemini | null } = { ok: false, data: null };
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Candado: o viene del propio servidor con su firma, o viene de una sesión
    // del dueño del proyecto o del equipo. Sin esto cualquiera podía disparar
    // el motor y quemar la cuota de Gemini.
    if (!(await puedeCorrerElMotor(req, String(id_proyecto)))) {
...
`;
    ...
    const partesUsuario: ParteGemini[] = [];
    ...
    return NextResponse.json(
      { error: "Error al procesar el proyecto", detalle: error?.message || String(err) },
      { status: 500 }
    );
  }
}
