...
Analiza el documento adjunto y complétalo contra esta lista de ${pasos.length} pasos de estructuración:
    ...
    const respuestaClaude = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: [contentBlock, { type: "text", text: instruccionFormato }],
          },
        ],
      }),
    });
