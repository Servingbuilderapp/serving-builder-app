...
export async function GET() {
  const password = process.env.SUPABASE_DB_PASSWORD || 'Serving2026*';
  const projectRef = process.env.SUPABASE_PROJECT_REF || 'ibcuyqoleuurfhnkwsgx';
  ...
  try {
    ...
    console.warn("DNS resolve6 failed, falling back to hostname:", err.message);
  }
  ...
  try {
    ...
    // 1. Execute Consolidated Migrations
...
`);
    ...
    // 2. Execute Presidencia Migrations
    console.log("Running presidencia migrations...");
    await client.query(`-- ====================================================================
...
('Circular Pack S.A.S.', 'Fondo Fomento de Ciencia y Tecnología Regional', 'Rechazado');
`);
    console.log("Presidencia migrations executed successfully.");

    // 3. Execute Convocatorias Migrations
    console.log("Running convocatorias migrations...");
    await client.query(`-- ====================================================================
...
`);
    console.log("Convocatorias migrations executed successfully.");

    return NextResponse.json({ 
      success: true, 
      message: 'Database migrated and seeded successfully in production!' 
    });
  } catch (err: any) {
    console.error("Migration error in GET:", err);
