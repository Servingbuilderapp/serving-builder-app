...
export function VentaCursoClient({ curso }: { curso: Curso }) {
  ...
  const montoUsd = curso.precioUsd
  const montoCop = precioCOP(curso)
  ...
  const handleConfirmar = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const res = await fetch('/api/academia/crear', {
        method: 'POST',
        ...
        <div className="max-w-xl mx-auto">
          <GlassCard className="p-7 md:p-9">
            <form onSubmit={handleConfirmar} className="space-y-6">
              <div>
                <h1 className="text-xl font-black text-color-base-content">{curso.nombre}</h1>
                <p className="text-color-base-content/60 text-sm mt-1">{curso.resumen}</p>
                <div className="text-2xl font-black text-color-primary mt-3">
          ...
          <div className="text-center space-y-6">
            ...
            {pais === 'colombia' && COBRO_COLOMBIA.modo === 'transferencia' && COBRO_COLOMBIA.cuenta ? (
              ...
              <div className="text-left p-6 rounded-2xl bg-color-base-content/5 border border-color-base-content/10 space-y-2">
                <p className="text-sm font-bold text-color-base-content">Valor a pagar: {precioUSDTexto(curso)}</p>
