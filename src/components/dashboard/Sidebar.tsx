    ...
    <div className="flex h-full flex-col glass-sidebar relative overflow-hidden border-r-0">
      ...
      <div className="flex h-16 shrink-0 items-center justify-between px-4 border-b border-color-base-content/5">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <span className={cn(
            ...
            {profile?.branding?.name ? profile.branding.name.toUpperCase() : 'ARQUITECTURA DIGITAL'}
          </span>
        </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1 custom-scrollbar">
        ...
        >
          ...
          <LucideIcons.Lightbulb className={cn(
            "h-5 w-5 shrink-0 transition-colors",
            pathname === '/ideas' ? "text-color-primary" : "text-color-base-content/40 group-hover:text-color-base-content"
          )} />
          <span className={cn("whitespace-nowrap transition-all duration-300", collapsed && "lg:hidden")}>
            {language === 'en' ? 'Idea Generator' : 'App de Ideas'}
          </span>
        </Link>
