 ...
 *
 * Todos son modelos estables. El primero es el que queremos; los siguientes son
 * el paracaidas. Google reparte el cupo por modelo, asi que cuando uno se agota
 * o se congestiona, los demas siguen disponibles. Antes, agotarse un modelo
 * dejaba parada la plataforma entera.
 */
  ...
  for (const modelo of MODELOS_GEMINI) {
    try {
      ...
      if (!response.ok) {
        ...
        // 503 = modelo saturado, 429 = cupo agotado. Los dos se resuelven
        // probando el siguiente modelo, que tiene su propio cupo.
